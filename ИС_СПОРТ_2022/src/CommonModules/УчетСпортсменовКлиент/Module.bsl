...
Процедура УчебныйГодСоздание(Форма, СтандартнаяОбработка) Экспорт
	
	СтандартнаяОбработка = Ложь;
	Оповещение = Новый ОписаниеОповещения("УчебныйГодСозданиеЗавершение", Форма.ЭтотОбъект);
	ОткрытьФорму("Обработка.МастерЗаполненияУчебногоГода.Форма.Форма",,,,,,Оповещение);
	
КонецПроцедуры

Процедура УчебныйГодОткрытие(УчебныйГод,СтандартнаяОбработка) Экспорт
	
	СтандартнаяОбработка = Ложь;
	ПоказатьЗначение(,УчебныйГод);
	
КонецПроцедуры

Процедура УчебныйГодНачалоВыбора(ЭтотОбъект,СтандартнаяОбработка) Экспорт
	
	СтандартнаяОбработка = Ложь;
	Оповещение = Новый ОписаниеОповещения("УчебныйГодНачалоВыбораЗавершение",ЭтотОбъект);
	ОткрытьФорму("Документ.УправлениеУчебнымГодом.ФормаВыбора",,,,,,Оповещение);
...
Процедура ВводМесяцаПриИзменении(РедактируемыйОбъект, ПутьРеквизита, ПутьРеквизитаПредставления, Модифицированность = Ложь) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(РедактируемыйОбъект, ПутьРеквизитаПредставления, УчетСпортсменовОбщегоНазначенияКлиентСервер.ПолучитьПредставлениеМесяца(Значение));
...
Процедура ВводМесяцаНачалоВыбораЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(РедактируемыйОбъект, ПутьРеквизита, Значение);
		Представление = УчетСпортсменовОбщегоНазначенияКлиентСервер.ПолучитьПредставлениеМесяца(Значение);
...
&НаКлиенте
Процедура УстановитьЗаголовокФормы(Форма, Спортсмен, ТекстФормы, Падеж)Экспорт
	Форма.Заголовок = УчетСпортсменовВызовСервера.СформироватьЗаголовокФормы(Спортсмен, ТекстФормы, Падеж);
КонецПроцедуры
