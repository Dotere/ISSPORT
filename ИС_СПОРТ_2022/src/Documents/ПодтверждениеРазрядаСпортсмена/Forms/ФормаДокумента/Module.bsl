...
Функция ПроверитьДатуПодтверждения()
	...
	КолонкаСпортсмен = Объект.Спортсмены.Выгрузить(, "Спортсмен, ДатаПодтверждения, ДатаСледующегоПодтверждения");
	
	Для Каждого Строка Из КолонкаСпортсмен Цикл
		Если Строка.ДатаПодтверждения = Дата(1, 1, 1) Тогда
			Строка.ДатаСледующегоПодтверждения = ДобавитьМесяц(Строка.ДатаПодтверждения, 24);
			Возврат Строка.ДатаСледующегоПодтверждения;
		КонецЕсли;
	КонецЦикла;
	
КонецФункции
// }Рарус adilas #10010 -Переделать документы для указания нескольких спортсменов 2020.10.12
...
&НаКлиенте
Процедура СпортсменыДатаПодтвержденияПриИзменении(Элемент)
	// Вставить содержимое обработчика.
	ЗаполнитьСрокДействияКлиент();
КонецПроцедуры
// }Рарус adilas #10010 -Переделать документы для указания нескольких спортсменов 2020.10.07
...
Процедура СпортсменыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование) 
	...
	Если ЗначениеЗаполнено(Объект.ДатаПодтверждающегоДокумента) Тогда
		Если НоваяСтрока Тогда
			Элемент.ТекущиеДанные.ДатаПодтверждения = Объект.ДатаПодтверждающегоДокумента;
...
Процедура ЗаполнитьДатой(Команда)
    ...
    ПоказатьВводДаты(Оповещение, , "Введите дату подтверждения");
...
Процедура ПослеВводаДаты(Дата, Параметры) Экспорт
	...
	Если Не Дата = Неопределено Тогда
		...
		Для Каждого Строка Из Объект.Спортсмены Цикл
			Строка.ДатаПодтверждения = Дата;
			Строка.ДатаСледующегоПодтверждения = ЗаполнитьСрокДействияСервер(Строка.Разряд, Дата);  
		КонецЦикла;
...
Процедура СогласиеНаОбработкуПерсональныхДанныхПолученоЗавершение(Результат, ДопПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ПараметрыФормы = Новый Структура("Субъект, ДатаСогласия", Объект.Спортсмен, Объект.ДатаПодтверждения);
...
Процедура ЗаполнитьСрокДействияКлиент()
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Разряд) И ЗначениеЗаполнено(ТекущиеДанные.ДатаПодтверждения) Тогда
		
		ТекущиеДанные.ДатаСледующегоПодтверждения = ЗаполнитьСрокДействияСервер(ТекущиеДанные.Разряд, ТекущиеДанные.ДатаПодтверждения);
...
&НаСервере
Функция ЗаполнитьСрокДействияСервер(Разряд, ДатаПодтверждения)

	ПродолжительностьЛет = Разряд.СрокДействия;
	МесяцевВГоду = 12;
	ЧислоМесяцев = МесяцевВГоду * ПродолжительностьЛет;
	ДатаСледующегоПодтверждения = НачалоДня(ДобавитьМесяц(ДатаПодтверждения, ЧислоМесяцев) -1);
	
	Возврат ДатаСледующегоПодтверждения;
