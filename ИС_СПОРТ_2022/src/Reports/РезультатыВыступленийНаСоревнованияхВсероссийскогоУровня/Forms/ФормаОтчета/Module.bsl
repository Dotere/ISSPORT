...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	// {Рарус adilas #17294 -Отчет "Результаты  выступлений на соревнованиях всероссийского уровня" 2021.11.17
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	//////////////////////////////////
	ТиповаяРасшифровка = ТиповаяРасшифровкаВозможнаНаСервере(Расшифровка);
		
	Если ТиповаяРасшифровка Тогда
	...
	Иначе
		...
		ПараметрФормыОрганизация = Неопределено;
		ПараметрФормыВидСпорта = Неопределено; 
		ПараметрФормыНачалоПериода = Неопределено;
		ПараметрФормыКонецПериода = Неопределено;
		
		ПолучениеПараметровОтчета(ПараметрФормыНачалоПериода, ПараметрФормыКонецПериода);
		
		СтруктураЗначенийРасшифровки = ПолучитьРасшифровкуНаСервере(Расшифровка);
		...
		Если УчетСпортсменовВызовСервера.ЭтоРуководитель() Тогда
			...
			ПараметрыФормы.Вставить("ВидСпорта", ПараметрФормыВидСпорта);
		КонецЕсли;
		
		ПараметрыФормы.Вставить("НачалоПериода", ПараметрФормыНачалоПериода);
		ПараметрыФормы.Вставить("КонецПериода", ПараметрФормыКонецПериода); 
		ПараметрыФормы.Вставить("ИмяПоляДляРасшифровки", СтруктураЗначенийРасшифровки.ИмяПоляДляРасшифровки);
		ПараметрыФормы.Вставить("ИмяПоляГруппировкиДляРасшифровки", СтруктураЗначенийРасшифровки.ИмяГруппировкиДляРасшифровки);
		ПараметрыФормы.Вставить("ВидСпортаДляРасшифровки", СтруктураЗначенийРасшифровки.ВидСпортаДляРасшифровки);
		
		ОткрытьФорму("Отчет.РезультатыВыступленийНаСоревнованияхВсероссийскогоУровня.Форма.ФормаРасшифровки", ПараметрыФормы, ЭтаФорма);
		// }Рарус adilas #17294 -Отчет "Результаты  выступлений на соревнованиях всероссийского уровня" 2021.11.17	
...
&НаСервере
Функция ТиповаяРасшифровкаВозможнаНаСервере(Расшифровка) 
	
	СтруктураЗначенийРасшифровки = Новый Структура;          
	
	КоллекцияРасшифровки = ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки);
	ЗначенияПолейРасшифровки = КоллекцияРасшифровки.Элементы.Получить(Расшифровка).ПолучитьПоля();
	
	Если ТипЗнч(ЗначенияПолейРасшифровки[0].Значение) = Тип("СправочникСсылка.ВидыСпорта") 
		ИЛИ ТипЗнч(ЗначенияПолейРасшифровки[0].Значение) = Тип("СправочникСсылка.РеестрВидовСпорта") 
		ИЛИ ТипЗнч(ЗначенияПолейРасшифровки[0].Значение) = Тип("СправочникСсылка.Организации") Тогда
...
&НаСервере
Процедура ПолучениеПараметровОтчета(ПараметрФормыНачалоПериода, ПараметрФормыКонецПериода)
	
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");		
			
	ПараметрФормыНачалоПериода = ОтчетОбъект.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.ДатаНачала;
	ПараметрФормыКонецПериода  = ОтчетОбъект.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.ДатаОкончания;
...
Функция ПолучитьРасшифровкуНаСервере(Расшифровка)
	...
	ЧислоРасшифровка = Число(Расшифровка);
	
	Найдено = Ложь;
	Пока ЧислоРасшифровка <> 0 И Не Найдено Цикл 
		
		МассивРодЭлементов = КоллекцияРасшифровки.Элементы.Получить(ЧислоРасшифровка).ПолучитьРодителей();
		
		Попытка 	
			Значение = МассивРодЭлементов[0].ПолучитьПоля();
			Если Значение.Количество() > 0 Тогда		
				ГруппировкаПоля = Значение[0];
				Найдено = Истина;		
			Иначе	
				Найдено = Ложь;	
			КонецЕсли;	
		...
		ЧислоРасшифровка = ЧислоРасшифровка - 1;
		
	КонецЦикла;
	
	ИмяПоляДляРасшифровки = ЗначенияПолейРасшифровки[0].Поле;
	ЗначениеПоляДляРасшифровки = ЗначенияПолейРасшифровки[0].Значение;
	ИмяГруппировкиДляРасшифровки = ГруппировкаПоля.Поле;
	ЗначениеГруппировкиДляРасшифровки = ГруппировкаПоля.Значение; 
	...
	Если РольДоступна("Методист") Тогда
		...
		Пока НЕ Найдено Цикл
			Если ТипЗнч(КоллекцияРасшифровки.Элементы.Получить(Расшифровка - ТекНомер).ПолучитьПоля()[0].Значение) <> Тип("СправочникСсылка.ВидыСпорта") Тогда
				
				ТекНомер = ТекНомер + 1;
				Найдено = Ложь;	
			Иначе	
				ЗначениеПоляВидСпортаДляРасшифровки = КоллекцияРасшифровки.Элементы.Получить(Расшифровка - ТекНомер).ПолучитьПоля()[0].Значение;	
				Найдено = Истина;
	...
	Иначе
		...
		Пока НЕ Найдено Цикл
			Если ТипЗнч(КоллекцияРасшифровки.Элементы.Получить(Расшифровка - ТекНомер).ПолучитьПоля()[0].Значение) <> Тип("СправочникСсылка.РеестрВидовСпорта") Тогда
	...
	СтруктураЗначенийРасшифровки.Вставить("ИмяПоляДляРасшифровки", ИмяПоляДляРасшифровки);
	СтруктураЗначенийРасшифровки.Вставить("ИмяГруппировкиДляРасшифровки", ЗначениеГруппировкиДляРасшифровки);
	СтруктураЗначенийРасшифровки.Вставить("ВидСпортаДляРасшифровки", ЗначениеПоляВидСпортаДляРасшифровки);
	
	Возврат СтруктураЗначенийРасшифровки;
