...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	// {Рарус adilas #23044 -Отбор. Этап спортивной подготовки. 2021.12.22
	// Перенесено из нетиповой формы, для единообразного отбора по этапу
	Если РольДоступна("Методист") ИЛИ ПривилегированныйРежим() Тогда
		ПолеОтбораВидСпортаПоРеестру = Новый ПолеКомпоновкиДанных("ВидСпортаПоРеестру");
    ...
    ИначеЕсли РольДоступна("Руководитель") ИЛИ РольДоступна("ПолныеПрава")  ИЛИ ПривилегированныйРежим() Тогда
	    ПолеОтбораВидСпорта = Новый ПолеКомпоновкиДанных("ВидСпорта");
