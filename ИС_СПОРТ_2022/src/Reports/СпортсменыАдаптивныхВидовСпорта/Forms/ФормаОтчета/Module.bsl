...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	// {Рарус adilas #17292 -Отчет "Спортсмены адаптивных видов спорта" 2021.07.23
	///////////////////////////////////////////////////////////////////////////
	// Типовой код
	...
	ПараметрФормыПериод = Неопределено;
	
	ПолучениеПараметровОтчета(ПараметрФормыОрганизация, ПараметрФормыПериод);
	СтруктураЗначенийРасшифровки = ПолучитьРасшифровкуНаСервере(Расшифровка);

	ПараметрыФормы = Новый Структура;
	
	ПараметрыФормы.Вставить("СформироватьПриОткрытии", Истина);
	// {Рарус dotere #23028 -Организация берется из параметров 2022.01.18
	ПараметрыФормы.Вставить("Организация", СтруктураЗначенийРасшифровки.ЗначениеПоляДляРасшифровкиОрганизация);
	// }Рарус dotere #23028 -Организация берется из параметров 2022.01.18
	ПараметрыФормы.Вставить("Период", ПараметрФормыПериод);
    ...
    ОткрытьФорму("Отчет.СпортсменыАдаптивныхВидовСпорта.Форма.ФормаРасшифровки", ПараметрыФормы, ЭтаФорма);
	// }Рарус adilas #17292 -Отчет "Спортсмены адаптивных видов спорта" 2021.07.23
...
// {Рарус adilas #17292 -Отчет "Спортсмены адаптивных видов спорта" 2021.10.21
#Область Расшифровка

&НаСервере
Процедура ПолучениеПараметровОтчета(ПараметрФормыОрганизация, ПараметрФормыПериод)
	
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
	
	ПараметрФормыОрганизация = ПараметрыСеанса.ТекущаяОрганизация;
	ПараметрФормыПериод = ОтчетОбъект.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Значение.Дата;
...
Функция ПолучитьРасшифровкуНаСервере(Расшифровка)
	...
	// {Рарус dotere #23028 -Организация берется из параметров 2022.01.18
	ИмяПоляДляРасшифровкиОрганизация = ЗначенияПолейРасшифровки[1].Поле;
	ЗначениеПоляДляРасшифровкиОрганизация = ЗначенияПолейРасшифровки[1].Значение;
	// }Рарус dotere #23028 -Организация берется из параметров 2022.01.18
	...
	// {Рарус dotere #23028 -Организация берется из параметров 2022.01.18
	СтруктураЗначенийРасшифровки.Вставить("ЗначениеПоляДляРасшифровкиОрганизация", ЗначениеПоляДляРасшифровкиОрганизация);
	// }Рарус dotere #23028 -Организация берется из параметров 2022.01.18
